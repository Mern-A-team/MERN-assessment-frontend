			...
			<div id='main'>
				...
				<div id='resultsContainer'>
					{results &&
						...
						results.map(photo => (
							<div key={photo.name} className='card'>
								<img
									src={`https://archivise.imgix.net/${photo.fileName}?auto=compress&w=150&q=30`}
									alt=''
								/>
								...
								<img src='' alt='' />
								<p>
									<span>Categories:</span> {photo.category.join(', ')}
								</p>
								<p>Description: {photo.description.substring(0, 25)}...</p>
							</div>
						))}
					...
					{!results &&
						...
						allPhotos.map(photo => (
							<div key={photo.name} className='card'>
								<img src={`https://archivise.imgix.net/${photo.fileName}?auto=compress&w=150&q=30`} alt='' />
								<Link key={photo.id} to={{ pathname: `/photo/${photo._id}` }}>
									{photo.name}
								</Link>
