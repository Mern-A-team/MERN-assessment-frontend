...
export default function Categories(props) {
  ...
  return (
    <>
      ...
      { data && popup === "" && 
        options.map(cat => <p>{cat}<button onClick={RenderEditCategory} value={cat} >Edit</button></p>)
        }
      ...
      { popup === "edit" &&
        <EditCategoryForm Close={CloseEdit} selectoptions={options} data={data} current={current} />
      }
    </>
  )
}
